...
export default function KioskTable({ visitorLogs, isHistoryMode }: { visitorLogs: any, isHistoryMode: boolean }) {
    ...
    return (
        <>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-sm font-bold text-slate-800 leading-tight">
                                {isHistoryMode ? "Historical Visitor Logs" : "Currently Active Visitors"}
                            </h2>
                            <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                                {isHistoryMode ? "A complete record of everyone who has visited." : "People who have signed in but haven't signed out yet."}
                            </p>
                        </div>
                    </div>
                    {!isHistoryMode && (
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {visitorLogs.data.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={5} className="h-40 text-center text-xs text-stone-400 font-medium">
                                        <Icon icon="solar:ghost-smile-bold-duotone" className="w-10 h-10 text-emerald-200 mx-auto mb-2" />
                                        {isHistoryMode ? "No historical records found." : "No one is currently logged into the library."}
                                    </TableCell>
                                </TableRow>
                            ) : (
                                visitorLogs.data.map((log: any) => (
                                    <TableRow key={log.id} className="hover:bg-emerald-50/30 transition-colors border-emerald-50">
                                        <TableCell className="pl-6 py-3">
                                            <p className="font-bold text-slate-800 text-sm">{log.visitor_name}</p>
                                            <div className="text-[10px] text-stone-500 font-medium mt-1 space-y-0.5">
                                                <p className="flex items-center gap-1.5">
                                                    <Icon icon="solar:map-point-bold-duotone" className="w-3 h-3 text-emerald-400" /> {log.address}
                                                </p>
                                                {log.school && (
                                                    <p className="flex items-center gap-1.5 text-emerald-600">
                                                        <Icon icon="solar:buildings-bold-duotone" className="w-3 h-3 text-emerald-500" /> {log.school}
                                                    </p>
                                                )}
                                            </div>
                                        </TableCell>
                                        ...
                                        <TableCell className="py-3">
                                            <div className="flex items-center text-sm font-bold text-slate-700">
                                                <Icon icon="solar:clock-circle-bold-duotone" className="w-4 h-4 mr-1.5 text-emerald-500" />
                                            ...
                                            <div className="text-[10px] text-stone-400 font-medium mt-1 tracking-wide">
                                                {new Date(log.time_in).toLocaleDateString([], { month: "short", day: "numeric", year: "numeric" })}
                                            </div>
                                        </TableCell>
                                        <TableCell className="py-3">
                                            {log.time_out ? (
                                                <div>
                                                    <div className="flex items-center text-sm font-bold text-slate-700">
                                                        <Icon icon="solar:stopwatch-bold-duotone" className="w-4 h-4 mr-1.5 text-stone-400" />
                                                        {new Date(log.time_out).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                                                    </div>
                                                    <div className="text-[10px] text-stone-400 font-medium mt-1 tracking-wide">
                                                        {new Date(log.time_out).toLocaleDateString([], { month: "short", day: "numeric", year: "numeric" })}
                                                    </div>
                                                </div>
                                            ) : (
                ...
                {visitorLogs.links && visitorLogs.total > 15 && (
                    <div className="bg-slate-50 border-t border-stone-100 px-6 py-3 flex items-center justify-between gap-4 flex-wrap">
                        <p className="text-[11px] text-stone-500 font-medium">
                            Showing <span className="font-bold text-slate-700">{visitorLogs.from}</span> to <span className="font-bold text-slate-700">{visitorLogs.to}</span> of <span className="font-bold text-slate-700">{visitorLogs.total}</span> records
                        </p>
            ...
            {/* CUSTOM FORCE LOGOUT MODAL */}
            {/* Note: Left as red/rose styling because a 'Force Logout' is slightly destructive */}
                ...
                <DialogContent className="sm:max-w-sm bg-white rounded-2xl border-rose-100 shadow-xl shadow-stone-200/50">
                    <DialogHeader>
                        <DialogTitle className="text-rose-600 font-black text-lg flex items-center gap-2">
                            <Icon icon="solar:danger-triangle-bold-duotone" className="w-6 h-6" /> Force Logout
                        </DialogTitle>
                        <DialogDescription className="text-xs text-slate-500 font-medium mt-2">
                            Are you sure you want to forcibly log this user out? Their time out will be recorded as right now.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className="pt-4">
                        ...
                        >
                            {processingId !== null ? "Logging out..." : "Confirm Logout"}
                        </button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
