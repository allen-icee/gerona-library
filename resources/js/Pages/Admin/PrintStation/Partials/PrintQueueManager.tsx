...
const TailwindCheckbox = ({
    ...
    <label className="flex items-center cursor-pointer relative">
        ...
        <div className="w-5 h-5 rounded-lg border-2 border-pink-300 bg-white peer-checked:bg-pink-500 peer-checked:border-pink-500 flex items-center justify-center transition-all">
            <svg
                className={`w-3.5 h-3.5 text-white ${checked ? "block" : "hidden"}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={3}
...
export default function PrintQueueManager({ queue }: { queue: PrintJob[] }) {
    ...
    const isGroupSelected = (jobs: PrintJob[]) =>
        jobs.length > 0 && jobs.every((j) => selectedJobIds.includes(j.id));

    const handleSelectGroup = (jobs: PrintJob[]) => {
        if (isGroupSelected(jobs)) {
            setSelectedJobIds((prev) =>
                prev.filter((id) => !jobs.find((j) => j.id === id)),
            );
        } else {
            const newIds = jobs
                .map((j) => j.id)
    ...
    const openLogModal = (
        ...
        const toLog = jobs.filter((j) => selectedJobIds.includes(j.id));
        if (toLog.length === 0) return;
        setActiveJobs(toLog);
        const estimatedPapers = toLog.reduce(
            (total, job) => total + (Number(job.copies) || 1),
            0,
        );
    ...
    const openDiscardModal = (jobs: PrintJob[]) => {
        const toDiscard = jobs.filter((j) => selectedJobIds.includes(j.id));
    ...
    const submitPrintLog = (e?: React.FormEvent) => {
        ...
        post(route("print-queue.log"), {
            ...
            onSuccess: () => {
                ...
                setSelectedJobIds((prev) =>
                    prev.filter((id) => !activeJobs.find((j) => j.id === id)),
                );
                reset();
            },
            onError: () => {
            ...
            {groupedQueue.length === 0 ? (
                <div className="border-dashed border-2 border-pink-200 bg-pink-50/50 rounded-2xl flex flex-col items-center justify-center py-16 text-pink-400">
                    <Icon
                        ...
                        className="w-12 h-12 mb-3 text-pink-300"
                    />
                ...
                <div className="space-y-4">
                    {groupedQueue.map((group) => {
                        const groupSelectedCount = group.jobs.filter((j) =>
                            selectedJobIds.includes(j.id),
                        ).length;

                        return (
                            <div
                                key={group.visitor_name}
                                className="bg-white border border-pink-100 shadow-sm shadow-pink-100/50 rounded-xl overflow-hidden"
                            >
                                            ...
                                            {group.jobs.map((job) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-4 py-2.5 text-center">
                                                        <span className="bg-pink-100 text-pink-700 px-2 py-0.5 rounded text-[10px] font-black">
                                                            x{job.copies}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-2.5 text-right">
                                                        ...
                                                        <a
                                                            ...
                                                            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-pink-500 bg-pink-50 hover:bg-pink-500 hover:text-white border border-pink-100 rounded-lg transition-all"
                                                            title="Download"
                                                        >
                                                            <Icon
                ...
                <DialogContent className="sm:max-w-sm bg-white rounded-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-xs text-slate-500">
                            Enter the total papers used for the{" "}
                    ...
                    <form onSubmit={submitPrintLog} className="space-y-4 py-2">
                        <div className="space-y-1.5">
                            <Label
                                ...
                                className="text-xs font-bold uppercase text-slate-600 tracking-wider"
                            >
                                Total Papers Used *
                            </Label>
                            <Input
                        ...
                        <DialogFooter className="pt-2">
                            ...
                            <button
                                type="submit"
                                disabled={processing}
                                className="px-4 py-2 text-sm font-bold bg-pink-500 hover:bg-pink-600 text-white rounded-lg shadow-sm shadow-pink-200 disabled:opacity-50"
                            >
                                {processing
                                    ? "Processing..."
                                    : "Confirm & Clear"}
                ...
                <DialogContent className="sm:max-w-sm bg-white rounded-2xl">
                    ...
                    <DialogFooter className="pt-2">
                        ...
                        <button
                            ...
                            className="px-4 py-2 text-sm font-bold bg-red-500 hover:bg-red-600 text-white rounded-lg shadow-sm shadow-red-200"
                        >
                            Discard Files
                        </button>
                    </DialogFooter>
