...
export default function BookActions({ book }: { book: any }) {
    ...
    const handleText = (val: string, field: any) => {
        if (/^[^<>*!@#$%^&]*$/.test(val)) setData(field, val);
    };
    const handleName = (val: string, field: any) => {
        if (/^[a-zA-Z\s.,'-]*$/.test(val)) setData(field, val);
    };
    const handleISBN = (val: string, field: any) => {
        if (/^[0-9X-]*$/i.test(val)) setData(field, val);
    };
    const handleYear = (val: string, field: any) => {
        if (/^\d{0,4}$/.test(val)) setData(field, val);
    };
        ...
        <>
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <Button
                        variant="ghost"
                        className="h-8 w-8 p-0 text-stone-400 hover:text-pink-600 hover:bg-pink-50 rounded-lg transition-colors"
                    >
                ...
                <DropdownMenuContent
                    align="end"
                    className="w-48 bg-white z-50 rounded-xl border-pink-100 shadow-lg shadow-stone-200/50"
                >
                    <DropdownMenuLabel className="text-xs font-bold uppercase text-stone-400 tracking-wider">
                        Actions
                    </DropdownMenuLabel>
                    <DropdownMenuItem
                    ...
                    <form onSubmit={submitEdit} className="space-y-4 py-2">
                        <div className="space-y-1.5">
                            ...
                            <Input
                                ...
                                value={data.title}
                                onChange={(e) =>
                                    handleText(e.target.value, "title")
                                }
                                onKeyDown={(e) => focusNext(e, "edit_author")}
                                required
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    value={data.author}
                                    onChange={(e) =>
                                        handleName(e.target.value, "author")
                                    }
                                    onKeyDown={(e) => focusNext(e, "edit_isbn")}
                                    required
                                    className="h-10 rounded-xl"
                                />
                            </div>
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    ...
                                    value={data.isbn}
                                    onChange={(e) =>
                                        handleISBN(e.target.value, "isbn")
                                    }
                                    onKeyDown={(e) =>
                                        focusNext(e, "edit_publisher")
                                    }
                                    className="h-10 rounded-xl"
                                />
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <Label
                                    htmlFor={`edit_publisher_${book.id}`}
                                    className="text-xs font-bold uppercase text-slate-600"
                                >
                                    Publisher
                                </Label>
                                <Input
                                    ...
                                    onChange={(e) =>
                                        handleName(e.target.value, "publisher")
                                    }
                                    onKeyDown={(e) => focusNext(e, "edit_year")}
                                    className="h-10 rounded-xl"
                                />
                            </div>
                            <div className="space-y-1.5">
                                <Label
                                    htmlFor={`edit_year_${book.id}`}
                                    className="text-xs font-bold uppercase text-slate-600"
                                >
                                    Year
                                </Label>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <Label
                                    htmlFor={`edit_category_${book.id}`}
                                    className="text-xs font-bold uppercase text-slate-600"
                                >
                                    Category
                                </Label>
                                <Input
                                    id={`edit_category_${book.id}`}
                                    value={data.category}
                                    onChange={(e) =>
                                        handleText(e.target.value, "category")
                                    }
                                    onKeyDown={(e) =>
                                        focusNext(e, "edit_language")
                                    }
                                    className="h-10 rounded-xl"
                                />
                            </div>
                            <div className="space-y-1.5">
                                <Label
                                    htmlFor={`edit_language_${book.id}`}
                                    className="text-xs font-bold uppercase text-slate-600"
                                >
                                    Language
                                </Label>
                                <Input
                                    id={`edit_language_${book.id}`}
                                    value={data.language}
                                    onChange={(e) =>
                                        handleText(e.target.value, "language")
                                    }
                ...
                <DialogContent className="sm:max-w-md bg-white rounded-2xl border-red-100 shadow-xl shadow-stone-200/50">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-stone-500 font-medium pt-2">
                            ...
                            This action will archive the book and it will no
