...
export default function AddBookModal() {
    ...
    const handleISBN = (val: string, field: any) => {
        if (/^[0-9xX-]*$/.test(val)) setData(field, val);
    };
    ...
    const submitNewBook: FormEventHandler = (e) => {
        ...
        post(route("books.store"), {
            ...
            onSuccess: () => {
                ...
                reset();
                toast.success(
                    `Master record and ${data.copies.length} copies added successfully!`,
                );
            },
            onError: () =>
                toast.error("Failed to save. Please check required fields."),
        });
    };
            ...
            <DialogContent className="sm:max-w-[40rem] bg-white rounded-2xl border-pink-100 shadow-xl shadow-stone-200/50 max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-xs text-slate-500 font-medium">
                        Create a new catalog entry and instantly register
                ...
                <form onSubmit={submitNewBook} className="space-y-4 py-2">
                    ...
                    <div className="pt-5 mt-4 border-t border-pink-100 relative z-20">
                        <div className="flex justify-between items-center mb-5 bg-pink-50/50 p-3 rounded-xl border border-pink-100">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <Input
                                    ...
                                    onChange={(e) =>
                                        handleNumCopiesChange(e.target.value)
                                    }
                        ...
                        <div className="space-y-4">
                            {data.copies.map((copy, index) => (
                                <div
                                    key={index}
                                    className="relative grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-white border border-stone-200 rounded-xl shadow-sm"
                        ...
                        {data.copies.length === 0 && (
                            <p className="text-center text-xs text-stone-400 font-bold py-4 bg-stone-50 rounded-xl border border-dashed border-stone-200">
                                0 Copies. The master record will be saved, but
                                no physical books will be added to inventory.
                            </p>
                        )}
                    </div>
                    ...
                    <DialogFooter className="pt-6 relative z-10 border-t border-slate-100 mt-6">
                        ...
                        >
                            {processing
                                ? "Saving..."
                                : `Save Record & ${data.copies.length} Copies`}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
