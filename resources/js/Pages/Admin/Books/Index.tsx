...
export default function BookIndex({
    ...
    recentBooks = [],
    filters,
}: PageProps<{ books: any; recentBooks?: any[]; filters: { search?: string } }>) {
    ...
    const recentLimit = 3;
    const displayedRecentBooks = isRecentExpanded ? recentBooks : recentBooks.slice(0, recentLimit);
                ...
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white p-4 rounded-2xl border border-pink-100 shadow-sm shadow-pink-100/50">
                    ...
                    <div className="flex flex-col md:flex-row w-full xl:w-auto items-stretch md:items-center gap-3">
                        ...
                        <div className="flex flex-col min-[480px]:flex-row w-full gap-2 shrink-0">
                            <a href={route("books.export")} className="flex-1 min-[480px]:flex-none flex items-center justify-center px-4 h-10 text-xs font-bold transition-all bg-pink-50 text-pink-600 hover:bg-pink-500 hover:text-white rounded-xl border border-pink-200 shadow-sm group whitespace-nowrap">
                ...
                {recentBooks.length > 0 && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex items-center gap-2 mb-3 px-1">
                            <Icon icon="solar:clock-circle-bold-duotone" className="w-5 h-5 text-emerald-500" />
                            <h2 className="text-sm font-black text-slate-800 tracking-tight uppercase">Today's Recent Inputs</h2>
                        </div>
                        ...
                        <div className="w-full bg-emerald-50/40 rounded-2xl border border-emerald-100 shadow-sm overflow-hidden flex flex-col">
                            <div className="overflow-x-auto">
                                <Table className="w-full min-w-[800px]">
                                    <TableHeader className="bg-emerald-100/50">
                                        <TableRow>
                                            <TableHead className="font-bold text-emerald-700 uppercase text-[10px] tracking-wider pl-6">Title</TableHead>
                                            <TableHead className="font-bold text-emerald-700 uppercase text-[10px] tracking-wider">Author</TableHead>
                                            <TableHead className="font-bold text-emerald-700 uppercase text-[10px] tracking-wider">Accession No.</TableHead>
                                            <TableHead className="font-bold text-emerald-700 uppercase text-[10px] tracking-wider text-center">Copies Added</TableHead>
                                            <TableHead className="font-bold text-emerald-700 uppercase text-[10px] tracking-wider text-right pr-6">Time Added</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                            ...
                                            <TableRow key={`recent-${book.id}`} className="hover:bg-emerald-50/80 transition-colors border-emerald-100/50">
                                                ...
                                                <TableCell>
                                                    <span className="font-mono text-[10px] font-bold text-emerald-700 bg-white border border-emerald-200 px-2 py-0.5 rounded-md shadow-sm whitespace-nowrap">
                                                        {book.copies && book.copies.length > 0 ? book.copies[book.copies.length - 1].accession_number : "N/A"}
                                                    </span>
                                                </TableCell>
                            ...
                            {recentBooks.length > recentLimit && (
                                ...
                                >
                                    {isRecentExpanded ? (
                                        <>Hide extra entries <Icon icon="solar:alt-arrow-up-bold-duotone" className="w-4 h-4" /></>
                                    ) : (
                                        <>View all {recentBooks.length} recent entries <Icon icon="solar:alt-arrow-down-bold-duotone" className="w-4 h-4" /></>
                                    )}
                                </button>
                            )}
                        </div>
                        ...
                        <Table className="w-full min-w-[950px]">
                            <TableHeader className="bg-stone-50/50 border-b border-pink-100">
                                <TableRow>
                                    ...
                                    <TableHead className="text-[10px] uppercase text-stone-400 font-bold">Latest Accession No.</TableHead>
                            ...
                            <TableBody>
                                {books.data.length === 0 ? (
                                    <TableRow><TableCell colSpan={6} className="h-32 text-center text-xs font-bold text-stone-400 uppercase tracking-widest">No books found in catalog.</TableCell></TableRow>
                                ) : (
                                        ...
                                        <TableRow key={book.id} className="hover:bg-pink-50/30 transition-colors border-pink-50">
                                            ...
                                            <TableCell className="py-3">
                                                <span className="font-mono text-[10px] font-bold text-slate-600 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-md whitespace-nowrap">
                                                    {book.copies && book.copies.length > 0 ? book.copies[book.copies.length - 1].accession_number : "No Copies Yet"}
                                                </span>
                                            </TableCell>

                                            <TableCell className="text-center py-3"><span className={`px-2.5 py-0.5 rounded-md text-[10px] font-black border ${book.copies_count > 0 ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-red-50 text-red-600 border-red-200"} whitespace-nowrap`}>{book.copies_count} copies</span></TableCell>
