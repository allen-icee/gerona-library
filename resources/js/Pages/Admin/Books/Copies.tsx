                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-pink-100 shadow-sm shadow-pink-100/50">
                    <div className="space-y-1.5">
                        <Link
                            ...
                            className="text-xs font-bold text-pink-500 hover:text-pink-600 flex items-center bg-pink-50 px-2 py-1 rounded-md w-fit"
                        >
                            <Icon
                        ...
                        <p className="text-slate-500 text-sm font-medium">
                            By {book.author}{" "}
                            <span className="text-stone-300 mx-1">|</span> ISBN:{" "}
                            {book.isbn || "N/A"}
                        </p>
                    </div>
                            ...
                            <TableBody>
                                {copies.length === 0 ? (
                                    <TableRow>
                                        ...
                                        >
                                            No physical copies registered yet.
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    copies.map((copy) => (
                                        <TableRow
                                            key={copy.id}
                                            className="hover:bg-pink-50/30 border-pink-50"
                                        >
                                            ...
                                            <TableCell className="py-3">
                                                <span
                                                    className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-[10px] font-black border ${copy.status === "Available" ? "bg-emerald-50 text-emerald-600 border-emerald-200" : copy.status === "Borrowed" ? "bg-amber-50 text-amber-600 border-amber-200" : "bg-red-50 text-red-600 border-red-200"}`}
                                                >
                                                    {copy.status}
                                                </span>
                                            </TableCell>
