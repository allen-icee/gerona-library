...
export default function AdminLayout({ children }: PropsWithChildren) {
    ...
    return (
        <div className="min-h-screen bg-[#FFF0F5] font-sans text-stone-800 relative flex flex-col overflow-x-hidden">
            ...
            <div className="absolute top-3 right-4 md:top-4 md:right-8 z-50">
                <DropdownMenu>
                    ...
                    >
                        ...
                        <DropdownMenuItem
                            asChild
                            className="rounded-lg hover:bg-pink-50 cursor-pointer text-stone-600 text-xs font-semibold mt-1"
                        >
                            <Link
            ...
            {/* ================= MAIN CONTENT AREA (ROTATED FOLDER) ================= */}
            {/* Shifted dashboard UP by reducing pt-16 to pt-10 */}
            <div className="max-w-[100rem] mx-auto w-full flex-1 flex flex-row pt-10 md:pt-14 px-2 md:px-6 relative z-10 pb-4 md:pb-6">
                {/* COMPACT VERTICAL FOLDER TABS ON THE LEFT */}
                {/* Shifted tabs UP by reducing pt-10 to pt-6 */}
                <div className="flex flex-col space-y-1 md:space-y-1.5 pt-6 md:pt-8 items-end z-20 flex-shrink-0">
                    {navItems.map((item, index) => {
                        ...
                        return active ? (
                            /* ACTIVE TAB (Now stretches EVEN FURTHER when hovered!) */
                            <Link
                                key={index}
                                href={item.href}
                            ...
                            /* INACTIVE TAB (Multi-colored, stretches when hovered) */
                            <Link
                                key={index}
                                href={item.href}
                ...
                {/* FOLDER BODY (Maximum screen space!) */}
