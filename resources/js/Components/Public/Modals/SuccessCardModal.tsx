...
export default function SuccessCardModal({
    isOpen,
    onClose,
    patronData,
}: Props) {
    ...
    const shareCard = async () => {
        ...
        try {
            ...
            const file = new File(
                [blob],
                `GeronaLibraryCard_${patronData.library_card_number}.png`,
                { type: "image/png" },
            );
            ...
            if (navigator.share) {
                ...
                alert(
                    "Your browser doesn't support native sharing. Please use the Save button instead.",
                );
            }
        } catch (error) {
                ...
                <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-[28rem] overflow-hidden flex flex-col items-center p-8 relative animate-in zoom-in-95 slide-in-from-bottom-8 duration-700">
                    ...
                    <div className="bg-emerald-50 border border-emerald-100 text-emerald-800 text-xs font-bold px-5 py-3.5 rounded-2xl flex items-center gap-3 w-full shadow-sm">
                        <Icon
                            ...
                            className="w-8 h-8 shrink-0 text-emerald-500"
                        />
                        <p className="leading-relaxed">
                            A copy of this digital library card has also been
                            sent to your{" "}
                            <span className="font-black">@gmail.com</span>{" "}
                            address.
                        </p>
                    </div>
